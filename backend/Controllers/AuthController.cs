...
namespace NjalaAPI.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        public async Task<IActionResult> Register(RegisterDTO model)
        {
            ...
            // Validate role
            var role = model.Role?.Trim() ?? "Student";
            if (role != "Student" && role != "Admin")
                return BadRequest("Invalid role. Must be 'Student' or 'Admin'.");
            ...
            if (role == "Admin")
            {
                await _auditService.LogAsync("Register", $"New admin registered (pending approval): {user.Email}");
                return Ok(new { message = "Registration successful. Your account is pending approval from SuperAdmin. You will be notified once approved." });
            }
            else
            {
        ...
        public async Task<IActionResult> Login(LoginDTO model)
        {
            ...
            _ = Task.Run(async () =>
            {
                try
                {
                    await _emailService.SendLoginSuccessEmailAsync(
                        user.Email,
                        user.FullName,
                        DateTime.UtcNow);
                }
                ...
                {
                    // Log email failure but don't block login
        ...
        public async Task<IActionResult> GoogleLogin(GoogleLoginDTO model)
        {
            ...
            if (user == null)
            {
                user = new ApplicationUser
                {
                    ...
                    Role = "Student",
                    AvatarUrl = payload.Picture // Capture Google profile photo
                };
            ...
            return Ok(new
            {
                token,
                refreshToken = refreshTokenEntity.Token,
                user = new { user.Id, user.Email, user.FullName, user.AvatarUrl, role = roles.FirstOrDefault() }
            });
        }
        ...
        public async Task<IActionResult> RequestOtp([FromBody] RequestOtpDto dto)
        {
            ...
            // Capture needed data before background task to avoid using disposed scoped services
        ...
        public async Task<IActionResult> RequestPasswordReset([FromBody] RequestResetDto dto)
        {
            ...
            _ = Task.Run(async () =>
            {
                ...
                {
                    // Log email failure but don't block password reset request
